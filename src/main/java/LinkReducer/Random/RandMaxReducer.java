...
public class RandMaxReducer implements ILinkReducer {
        ...
        for(ArrayList<Alignment> inc : incompatibilities){
            ...
            if(inc.get(0).getMeasure() >= Float.parseFloat(config.get("specialValue")) && inc.get(1).getMeasure() < Float.parseFloat(config.get("specialValue"))){
                ...
                if (selectRandom(inc)) {
                    ...
                    Alignment a = new Alignment(inc.get(1).getSubject(), inc.get(1).getObject(), Math.max(inc.get(0).getMeasure(), inc.get(1).getMeasure()));
                    result.add(a);
                }
            }
        }
