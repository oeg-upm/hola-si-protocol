...
public class MaxMeanReducer implements ILinkReducer {
        ...
        for(ArrayList<Alignment> inc : incompatibilities){
            ...
            if(inc.get(0).getMeasure() >= Float.parseFloat(config.get("specialValue")) && inc.get(1).getMeasure() < Float.parseFloat(config.get("specialValue"))){
                ...
                Float measure = ((inc.get(0).getMeasure() - inc.get(1).getMeasure()) / 3) +
                        ((inc.get(0).getMeasure() + inc.get(1).getMeasure()) / 2);
                ...
                //If they have different measure, select the higher one and assign it the mean value of both, modificated to not be too punisher to the higher value
